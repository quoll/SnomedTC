...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "Usage: algorithm_a_ser <input_snomed_file> <output_file>\n";
        return 1;
    }
    ...
    try {
        ...
        t0 = Clock::now();
        AdjMap closure = compute_a_ser_closure(conn);
        t1 = Clock::now();
        ...
        std::cout << "Algorithm A-serial flatten_closure to pairs: "
                  << std::chrono::duration<double, std::milli>(t1 - t0).count() << " ms\n";
        std::cout << "Algorithm A-serial closure size: " << pairs.size() << "\n";
