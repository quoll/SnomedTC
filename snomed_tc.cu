...
// mapping of index values 0-N to the associated SNOMED id, on the GPU
    ...
    const DestMapping &mapping)
{
    ...
    for (const auto &e : external_edges) {
        auto it_dst = id_to_index.find(e.second);
        if (it_dst == id_to_index.end()) continue;
        ++csr.row_offsets[src_to_row[e.first] + 1];
    }
    ...
    for (const auto &e : external_edges) {
        auto it_dst = id_to_index.find(e.second);
        if (it_dst == id_to_index.end()) continue;
        int row = src_to_row[e.first];
        csr.dst_indices[cursor[row]++] = it_dst->second;
    }
...
ClosurePairs compute_external_closure_gpu(const BitsetMatrixDevice &closure_dev,
    ...
    // 3. Count pairs per external source
    ...
    // 6. Emit destination ids, re-computing the OR accumulation
