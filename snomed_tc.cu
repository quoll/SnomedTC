...
int main(int argc, char **argv) {
    ...
    try {
        ...
        // 8. Convert final closure (in->data) to (srcId, dstId) pairs
        t0 = Clock::now();
        ClosurePairs closureA_pairs = convert_closure_to_pairs(*in, mapping, external_edges);
        t1 = Clock::now();
        auto tA1 = t1;
        std::cout << "Algorithm A convert_closure_to_pairs: "
